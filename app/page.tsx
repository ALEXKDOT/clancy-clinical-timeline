...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <header className="topbar">
      <div className="brand-lockup"><div className="brand-mark" aria-hidden="true"><span /><span /><span /></div><div><p className="kicker">Longitudinal care, in context</p><h1>Clinical timeline <span>prototype</span></h1></div></div>
      <div className="prototype-tag"><span aria-hidden="true" />Curated demonstration</div>
    </header>

    <section className="patient-header" aria-labelledby="patient-name">
      <div className="patient-identity"><div className="patient-avatar" aria-hidden="true">LC</div><div><p className="eyebrow">Patient timeline</p><h2 id="patient-name">{patient.name}</h2><p>{patient.subtitle}</p></div></div>
      <div className="patient-period"><span className="eyebrow">Period of care</span><strong>{rangeLabel}</strong><span>Encounters · messages · treatment history</span></div>
    ...
    <div className="prototype-note"><span className="note-symbol" aria-hidden="true">i</span><p><strong>An EHR timeline concept.</strong> Manually curated clinical summaries showing how Epic / CareConnect records could be organized. This is not an EHR export and has no live Epic or CareConnect connection.</p></div>

    <section id="timeline-workspace" className="workspace" aria-label="Clinical record explorer">
      <div className="control-deck">
        <div className="workspace-heading"><div><p className="eyebrow">Explore the care journey</p><h2>Care over time.</h2></div><div className="view-switch" role="group" aria-label="Record view"><button className={view === "timeline" ? "active" : ""} aria-pressed={view === "timeline"} onClick={() => setView("timeline")}><span aria-hidden="true">↔</span> Timeline</button><button className={view === "list" ? "active" : ""} aria-pressed={view === "list"} onClick={() => setView("list")}><span aria-hidden="true">☷</span> Record list</button></div></div>
        <div className="tools-row">
          ...
          {view === "timeline" && <div className="zoom-control" role="group" aria-label="Timeline zoom"><button aria-label="Zoom out" disabled={!fit && zoom <= .35} onClick={() => changeZoom(fit ? .35 : zoom - .2)}>−</button><span>{fit ? "Overview" : `${Math.round(zoom * 100)}%`}</span><button aria-label="Zoom in" disabled={!fit && zoom >= 2} onClick={() => changeZoom(fit ? .55 : zoom + .2)}>+</button><button className={fit ? "fit active" : "fit"} aria-pressed={fit} onClick={() => { setFit(!fit); }}>Fit</button></div>}
          <button className="primary-button" disabled={!filteredEvents.length} onClick={() => selectEvent(filteredEvents[0])}><span aria-hidden="true">▷</span> Review entries</button>
        </div>
      ...
      <div className="timeline-status"><p aria-live="polite"><strong>{filteredEvents.length}</strong> of {events.length} entries{query && <> matching “{search}”</>}</p>{view === "timeline" ? <><span className="scroll-hint">Scroll horizontally · select a card for details</span><button className={`medication-toggle ${showMedications ? "active" : ""}`} aria-pressed={showMedications} onClick={() => setShowMedications(!showMedications)}><span aria-hidden="true">{showMedications ? "−" : "+"}</span> Medication lanes</button></> : <span>Chronological order</span>}</div>
